...
echo $upload_path."/".$default_image_name;?>?<?php echo time(); //to prevent browser image caching ?>" alt="Picture">
  </div>
...
$("#profile_pic_save").on("click", function() {
 ...
 var post_data = {
	 ...
	 $.ajax({
    type: 'POST',
    url: '<?php echo site_url('users/profiles/save_profile_pic')."/".$user_id;?>',
	contentType: "application/json; charset=utf-8",
     data: result.toDataURL('image/png')   ,
    success: function(response){
		  jAlert(response.message, 'Confirmation Dialog', function(r) {
						  $("#profile_avatar").attr("src","<?php echo $upload_path."/";?>"+response.new_image+"?"+d.getTime());
                         tb_remove();
                     });
