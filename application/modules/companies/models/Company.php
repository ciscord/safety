...
class Company  extends CI_Model
{
	...
	*/
	public function exists($company_id)
	{
		...
		$this->db->join('userinfo', 'userinfo.company_id = companies.company_id');
		$this->db->where('companies.company_id',$user_id);
		$query = $this->db->get();
...
GROUP BY ".$this->db->dbprefix."companies.company_id");
	...
	*/
	public function get_info($users_id)
	{
		$this->db->from('companies');	
		$this->db->join('userinfo', 'userinfo.user_id = companies.user_id');
		$this->db->where('companies.user_id',$users_id);
		$query = $this->db->get();
	...
	*/
	public function get_multiple_info($users_id)
	{
		$this->db->from('companies');
		$this->db->join('userinfo', 'userinfo.user_id = companies.user_id');		
		$this->db->where_in('companies.user_id',$users_id);
	...
	public function save(&$userinfo_data, &$userlog_data,&$permission_data,$user_id=false)
	{
		...
		if (parent::save($userinfo_data,$userlog_data,$permission_data,$user_id))
		{
			if (!empty($userlog_data)) {
				...
				$success = $this->db->insert('companies',$userlog_data);
			    }
			    else {
				$this->db->where('user_id', $user_id);
				$success = $this->db->update('companies',$userlog_data);		
			    }
	...
	*/
	public function get_search_suggestions($search,$limit=5)
	{
		...
		$this->db->select('first_name');
		$this->db->from('companies');
		$this->db->join('userinfo','companies.user_id=userinfo.user_id');		
