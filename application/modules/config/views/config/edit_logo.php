...
?>
<?php $upload_path=site_url("uploads"); ?>
  <div id="qunit"></div>
  <div id="qunit-fixture"></div>
  
  <h1 class="center"> Edit Avatar</h1>
...
$("#profile_pic_save").on("click", function() {
 ...
 var post_data = {
	 ...
	 $.ajax({
    type: 'POST',
    url: '<?php echo site_url('config/save_logo');?>',
	contentType: "application/json; charset=utf-8",
    data: result.toDataURL('image/png')   ,
    success: function(response){
		  jAlert(response.message, 'Confirmation Dialog', function(r) {
						  $("#app_logo").attr("src","<?php echo $upload_path."/";?>"+response.new_image+"?"+d.getTime());
                         tb_remove();
                     });
